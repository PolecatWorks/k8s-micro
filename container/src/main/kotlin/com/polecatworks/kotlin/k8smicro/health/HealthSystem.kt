...
open class HealthSystem {
    ...
    @OptIn(ExperimentalTime::class)
    open fun checkAlive(myNow: ValueTimeMark): HealthSystemResult {
        val results = alive.map { value -> value.check(myNow) }

        return HealthSystemResult("alive", results.all { result -> result.valid }, results)
    }

    open fun checkReady(myNow: ValueTimeMark): HealthSystemResult {
        val results = ready.map { value -> value.check(myNow) }

        return HealthSystemResult("ready", results.all { result -> result.valid }, results)
    }

    open suspend fun registerAlive(myHealth: IHealthCheck): Boolean {
        aliveAccess.withLock {
            return alive.add(myHealth)
        }
    }

    open suspend fun deregisterAlive(myHealth: IHealthCheck): Boolean {
        aliveAccess.withLock {
            return alive.remove(myHealth)
        }
    }

    open suspend fun registerReady(myHealth: IHealthCheck): Boolean {
        readyAccess.withLock {
            return ready.add(myHealth)
        }
    }

    open suspend fun deregisterReady(myHealth: IHealthCheck): Boolean {
        readyAccess.withLock {
